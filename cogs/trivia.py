    ...
    async def trivia(self, ctx, list_name : str=None):
        ...
        if list_name == None:
            ...
            if await get_trivia_by_channel(message.channel):
                ...
                await ctx.send("There's no trivia session ongoing in this channel.")
            ...
            await ctx.send("A trivia session is already ongoing in this channel.")
        ...
        if lists:
            ...
            if clean_list:
                ...
                if len(clean_list) > 100:
                    ...
                    await author.dm_channel.send(author, msg)
                else:
                    await author.create_dm()
                    await author.dm_channel.send(msg)
            else:
                await author.create_dm()
                await author.dm_channel.send("There are no trivia lists available.")
        else:
            await author.create_dm()
            await author.dm_channel.send("There are no trivia lists available.")
...
class TriviaSession():
    ...
    async def load_questions(self, msg):
        ...
        if len(msg) == 3:
            _, _, qlist = msg
            if qlist == "random":
                chosen_list = choice(glob.glob("data/trivia/*.txt"))
                ...
                if os.path.isfile("data/trivia/" + qlist + ".txt"):
                    ...
                    await self.channel.send("There is no list with that name.")
                    await self.stop_trivia()

        else:
            await self.channel.send("trivia [list name]")
    ...
    async def send_table(self , message):
        self.score_list = sorted(self.score_list, reverse=True, key=lambda x: x[1]) # orders score from lower to higher
        t = "```Scores: \n\n"
...
def check_class():
    settings = {"TRIVIA_MAX_SCORE" : 10, "TRIVIA_TIMEOUT" : 120,  "TRIVIA_DELAY" : 15, "TRIVIA_BOT_PLAYS" : True}
...
check_trivia()
check_class()
client.add_listener(check_messages, "on_message")
trivia_manager = Trivia(client)
client.add_cog(trivia_manager)
