...
class Database:
    ...
    async def connect(self):
        self._conn = await asyncpg.create_pool(user='postgres', password='kgvv2qd5Rh', database='pinkguy', host='127.0.0.1', port='8080')
    ...
    async def user_insert(self, member, data):
        ...
        req = f"""INSERT INTO market.userdata ("UUID", info) VALUES ({member.id}, '{jd}')"""
    ...
    async def user_select(self, member):
        ...
        req = f"""SELECT info -> '{member.guild.id}' FROM market.userdata WHERE "UUID" = {member.id}"""
        async with self._conn.acquire() as connection:
    ...
    async def user_full_select(self, member):
        ...
        """Select a user's data for a specified server"""
        req = f"""SELECT info FROM market.userdata WHERE "UUID"= {member.id}"""
        async with self._conn.acquire() as connection:
            response = await connection.fetchval(req)
        return json.loads(response) if response else response
    ...
    async def guild_insert(self, guild, data):
        ...
        jd = self.dump(data)
        req = f"""INSERT INTO market.guilddata ("UUID", info) VALUES ({guild.id}, '{jd}')"""
        async with self._conn.acquire() as connection:
            await connection.execute(req)
    ...
    async def guild_select(self, guild):
        ...
        req = f"""SELECT info FROM market.guilddata WHERE "UUID" = {guild.id}"""
        async with self._conn.acquire() as connection:
            response = await connection.fetchval(req)
        return json.loads(response) if response else response
    ...
    async def update_guild_data(self, guild, data):
        ...
        # upsert
        jd = self.dump(data)
        req = f"""INSERT INTO market.guilddata ("UUID", info)
        VALUES (
            {guild.id},
            '{jd}'
        )
    ...
    async def guild_item(self, guild, name: str):
        ...
        req = f"""SELECT info ->> '{name}' FROM market.guilddata WHERE "UUID" = {guild.id}"""
        async with self._conn.acquire() as connection:
            response = await connection.fetchval(req)
    ...
    async def user_item(self, member, name: str):
        ...
        req = f"""SELECT info -> '{member.guild.id}' ->> '{name}' FROM market.userdata WHERE "UUID" = {member.id}"""
        async with self._conn.acquire() as connection:
            response = await connection.fetchval(req)
