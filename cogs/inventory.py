...
from cogs.utils.data import MemberConverter, ItemOrNumber, chain, IntConverter, create_pages, parse_varargs, chunkn
...
class Inventory(commands.Cog):
    ...
    async def clearinv(self, ctx, *members: MemberConverter):
    ...
    @commands.command(invoke_without_command=True, aliases=['lb'])
    async def loot(self, ctx, name: str = None):
    ...
    async def exchange(self, ctx, other: discord.Member, *items: str):
        """Send a trade offer to another user."""
        self.trades[other] = (ctx, items)
        await ctx.send((await _(ctx, "{} has 5 minutes to respond to this request using ey trade respond. See ey help trade respond for details")).format(other))
    ...
    async def respond(self, ctx, other: discord.Member, *items: str):
        ...
        if sender in self.trades and other == self.trades[sender][0].message.author:
            await ctx.send(
                await _(ctx, "Both parties say ey accept @Other to accept the trade or ey decline @Other to decline"))
            already = None

            def check(message):
                if not (message.channel == ctx.channel):
                    return False
                if not message.content.startswith(("ey accept", "ey decline",)):
                    return False
            ...
            elif msg.content.startswith("ey decline"):
            ...
            elif msg2.content.startswith("ey decline"):
                await ctx.send(await _(ctx, "Trade declined, cancelling!"))
                del self.trades[sender]
                return
    ...
    @commands.command()
    @checks.mod_or_permissions()
    async def cook(self, ctx, *, name: str):
