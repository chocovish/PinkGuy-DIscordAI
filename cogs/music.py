    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            try:
                ...
                async with timeout(800):  # 5 minutes...
            ...
            self.np = await self._channel.send(f'**Now Screaming:** `{source.title}` requested by '
                                               f'`{source.requester}` faggot')
...
class Music(commands.Cog):
    ...
    async def __error(self, ctx, error):
        ...
        if isinstance(error, commands.NoPrivateMessage):
            ...
            await ctx.send('Lmao fag dont even know the voice channel name')
    ...
    async def play(self, ctx, *, search: str):
        ...
        await ctx.message.add_reaction(emoji=":SalamandarMan:666204128652951564")
    ...
    async def queue_info(self, ctx):
        ...
        embed = discord.Embed(title=f'Next Screamings {len(upcoming)}', description=fmt)
    ...
    async def now_playing_(self, ctx):
        ...
        player.np = await ctx.send(f'**Now Screaming:** `{vc.source.title}` '
                                   f'requested by `{vc.source.requester}` faggot ')
